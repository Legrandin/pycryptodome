...
__all__ = ['new', 'digest_size', 'SHA3_224Hash' ]

from Crypto.Util.py3compat import *
from Crypto.Hash.hashalgo import HashAlgo

from Crypto.Hash import _SHA3_224
hashFactory = _SHA3_224

class SHA3_224Hash(HashAlgo):
    """Class that implements a SHA3_224 hash
    ...
    #:
    #:
    #: This value uniquely identifies the SHA3_224 algorithm.
    oid = b('*-not yet assigned-*')

    digest_size = _SHA3_224.digest_size
    block_size = _SHA3_224.block_size

    def __init__(self, data=None):
        HashAlgo.__init__(self, hashFactory, data)

    def new(self, data=None):
        return SHA3_224Hash(data)
...
def new(data=None):
    ...
    :Parameters:
       data : byte string
        ...
        It is equivalent to an early call to `SHA3_224Hash.update()`.
        Optional.

    :Return: A `SHA3_224Hash` object
    """
    return SHA3_224Hash().new(data)

#: The size of the resulting hash in bytes.
digest_size = SHA3_224Hash.digest_size

#: The internal block size of the hash algorithm in bytes.
block_size = SHA3_224Hash.block_size
