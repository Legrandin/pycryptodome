...
/*
 * streamIV_template.c : Generic framework for stream ciphers with
 * Initialization Vector (IV)
 *
 * Written by Andrew Kuchling, Fabrizio Tarizzo and others
 *
 * ===================================================================
...
ALGnew(PyObject *self, PyObject *args, PyObject *kwdict)
{
	...
	new = newALGobject();
	if (!PyArg_ParseTupleAndKeywords(args, kwdict, "s#s#", kwlist, 
					 &key, &keylen, &IV, &IVlen))
	{
		Py_DECREF(new);
		return NULL;
	...
	stream_init(&(new->st), key, keylen, IV, IVlen);
	if (PyErr_Occurred())
	{
		Py_DECREF(new);
		return NULL;
